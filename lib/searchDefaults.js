...
const LIMITS = { queries: 6, positive: 40, negative: 40, excludeDomains: 50 };
...
// A saved settings doc with every list empty means "never configured" —
...
function effectiveSettings(saved) {
  ...
  const allEmpty = !s.queries.length && !s.positive.length && !s.negative.length && !s.excludeDomains.length;
...
module.exports = { ANCHOR, DEFAULT_SETTINGS, LIMITS, sanitizeList, normalizeSettings, effectiveSettings };
